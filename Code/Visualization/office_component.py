...
def showdataframe():
    ...
    df1 = pd.read_csv("weights_office.csv")
    df1 = df1.loc[:, ~df1.columns.str.contains('^Unnamed')]
    col1.dataframe(df1, width=1800)

    df2 = pd.read_csv("office.csv")
...
def render():
    ...
    if button1:
        ...
        df3 = pd.read_csv('ml_office.csv')
        fig1 = px.histogram(df3, x = 'num_purchase', color = 'type', histnorm = 'percent')
        st.plotly_chart(fig1)
    elif button2:
        df_office2 = pd.read_csv("weights_office.csv")
    ...
    dfN1 = pd.read_json('office_keywords.json')
    rows = dfN1.values
    pairs=[]
    for row in rows:
        for item in row:
